...
检查目录和编号连续性、必填字段、素材到页面的对应关系、工具 ID 是否来自公开目录、可选模板文件的数量与类型，以及未知文件。没有素材缺口时，使用当前环境可用的任意打包能力，在模板目录旁生成同名 `.zip`；不得假设系统存在 `zip` 命令，也不得把本 skill 的脚本当作唯一实现。保留原目录，ZIP 内必须满足：

- `course.yaml` 直接位于 ZIP 根目录；不得额外包一层课程目录。
- 仅包含契约允许的文件和目录；排除 `.DS_Store`、`__MACOSX/`、README、临时文件和系统隐藏文件。
- 条目名称为 UTF-8、NFC、相对路径，并使用 `/` 分隔；不含绝对路径、空路径段、`.`、`..`、反斜杠逃逸或符号链接。
- 使用标准 ZIP 2.0：仅 stored 或 deflate；不使用 ZIP64、加密条目、data descriptor 或未知压缩方法。
- 每个文件的 CRC、压缩大小和解压大小必须在 local header 与 central directory 中一致；不得设置 ZIP general-purpose flag 的加密位（bit 0）或 data-descriptor 位（bit 3）。

如当前环境具备 ZIP 元数据检查能力，检查 `course.yaml` 条目未加密、未使用 data descriptor，并确认其位于根目录。当前可用工具无法明确满足上述 ZIP 元数据约束时，停止并说明限制；不要生成或声称生成了可导入 ZIP。向用户报告目录和 ZIP 路径。
